...
const Pants = async () => {
  try {
    ...
    // Filter products by category 'pants'
    const filteredProducts = products.filter(
      (product: any) => product.category === "pants"
    );

    if (filteredProducts.length === 0) {
      // Handle case where no products are found in the 'pants' category
      return (
        <div>
          {/* Spacer or Banner */}
          ...
          <div className="w-full h-[150px] flex items-center bg-[#f6f4ee]">
            <p className="text-black p-8 text-2xl font-bold">Pants All Products</p>
          </div>

          {/* No Products Message */}
          <div className="mt-24 px-4 text-center">
            <p className="text-gray-500 text-lg">No pants available at the moment.</p>
          </div>
        </div>
      );
    }
