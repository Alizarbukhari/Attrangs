...
const OuterWear = async () => {
  try {
    ...
    // Filter products by category 'outerwear'
    const filteredProducts = products.filter(
      (product: any) => product.category === "outerwear"
    );

    if (filteredProducts.length === 0) {
      // Handle case where no products are found in the 'outerwear' category
      return (
        <div>
          {/* Spacer or Banner */}
          ...
          <div className="mt-24 px-4 text-center">
            <p className="text-gray-500 text-lg">No outerwear available at the moment.</p>
          </div>
        </div>
      );
    }
