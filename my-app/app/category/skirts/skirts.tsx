...
const Skirts = async () => {
  try {
    ...
    // Filter products by category 'skirts'
    const filteredProducts = products.filter(
      (product: any) => product.category === "skirts"
    );

    if (filteredProducts.length === 0) {
      // Handle case where no products are found in the 'skirts' category
      return (
        <div>
          {/* Spacer or Banner */}
          ...
          <div className="w-full h-[150px] flex items-center bg-[#f6f4ee]">
            <p className="text-black p-8 text-2xl font-bold">Skirts All Products</p>
          </div>

          {/* No Products Message */}
          <div className="mt-24 px-4 text-center">
            <p className="text-gray-500 text-lg">No skirts available at the moment.</p>
          </div>
        </div>
      );
    }
